...
<x-main.main-template>
    <div class="my-16 mx-10">
        <div class="flex justify-between mb-5">
            <h1 class="text-3xl font-bold">List Company</h1>
        ...
        duration-300"
                href="{{ route('create-company') }}">CREATE</a>
        </div>
                ...
                @foreach ($company as $item)
                    <tr
                        ...
                        <td
                            ...
                            <span
                                class="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">gambar</span>
                            {{ $item->gambar }}
                        </td>
                        <td
                            class="lg:flex lg:justify-center lg:items-center gap-x-4 w-full lg:w-auto p-3 text-gray-800 text-center relative lg:static">
                            <a class="my-2 border px-3 py-2 rounded-lg text-white text-sm bg-yellow-600"
                                href="{{ route('edit-company', $item->id) }}">edit</a>
                            <a class="my-2 border px-3 py-2 rounded-lg text-white text-sm bg-red-600"
                                href="{{ route('delete-company', $item->id) }}">delete</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
